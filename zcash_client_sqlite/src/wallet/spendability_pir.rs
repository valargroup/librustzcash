...
// =========================================================================
// Types — PIR client (network operations)
// =========================================================================

/// Error from a PIR client network operation.
#[cfg(feature = "spendability-pir")]
#[derive(Debug)]
pub enum PirClientError {
    /// Failed to connect to the PIR server.
...
#[cfg(feature = "spendability-pir")]
impl std::fmt::Display for PirClientError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PirClientError::ConnectionFailed(e) => write!(f, "PIR connection failed: {e}"),
            PirClientError::QueryFailed(e) => write!(f, "PIR query failed: {e}"),
        }
    }
}

#[cfg(feature = "spendability-pir")]
impl std::error::Error for PirClientError {}

/// Spend metadata returned by the PIR server for a matched nullifier.
#[cfg(feature = "spendability-pir")]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct PirSpendMetadata {
...
#[cfg(feature = "spendability-pir")]
#[derive(Debug)]
pub struct PirNullifierCheckResult {
    pub earliest_height: u64,
    pub latest_height: u64,
    /// Parallel to the input nullifiers: `Some(meta)` = spent, `None` = not spent.
    pub spent: Vec<Option<PirSpendMetadata>>,
}

/// Input for a PIR witness fetch: identifies a note by its database id and
/// commitment tree position.
#[cfg(feature = "spendability-pir")]
#[derive(Debug, Clone)]
pub struct PirWitnessRequest {
    pub note_id: i64,
    pub position: u64,
}

/// A witness entry returned from the PIR server.
#[cfg(feature = "spendability-pir")]
#[derive(Debug, Clone)]
pub struct PirFetchedWitness {
    pub note_id: i64,
    pub position: u64,
    pub siblings: [[u8; 32]; 32],
    pub anchor_height: u64,
...
// =========================================================================
// Functions — PIR client (network operations)
// =========================================================================

/// Connects to a PIR spend-server and checks whether any of the given
/// nullifiers have been spent on-chain.
///
/// This is a blocking network call. The `progress` callback receives values
/// in `0.0..=1.0` after each individual nullifier query completes.
#[cfg(feature = "spendability-pir")]
pub fn check_nullifiers_via_pir(
    server_url: &str,
    nullifiers: &[[u8; 32]],
    progress: impl Fn(f64),
) -> Result<PirNullifierCheckResult, PirClientError> {
    let client = spend_client::SpendClientBlocking::connect(server_url)
        .map_err(|e| PirClientError::ConnectionFailed(e.to_string()))?;

    let spent_raw = client
        .check_nullifiers(nullifiers, &progress)
        .map_err(|e| PirClientError::QueryFailed(e.to_string()))?;

    let metadata = client.metadata();
    let spent = spent_raw
        .into_iter()
        .map(|opt| {
            opt.map(|m| PirSpendMetadata {
                spend_height: m.spend_height,
                first_output_position: m.first_output_position,
                action_count: m.action_count,
            })
        })
        .collect();

    Ok(PirNullifierCheckResult {
...
/// Connects to a PIR witness-server and fetches Merkle authentication paths
/// for the requested note positions.
///
/// This is a blocking network call. The `progress` callback receives values
/// in `0.0..=1.0` after each individual witness query completes.
///
/// Returns a `Vec<PirFetchedWitness>` parallel to the input `requests`.
#[cfg(feature = "spendability-pir")]
pub fn fetch_witnesses_via_pir(
    server_url: &str,
    requests: &[PirWitnessRequest],
    progress: impl Fn(f64),
) -> Result<Vec<PirFetchedWitness>, PirClientError> {
    if requests.is_empty() {
        return Ok(vec![]);
    }

    tracing::info!(num_notes = requests.len(), url = %server_url, "PIR witness fetch: starting");
    ...
    let pir_witnesses = client
        .get_witnesses(&positions, &progress)
        .map_err(|e| PirClientError::QueryFailed(e.to_string()))?;
    tracing::info!(
        elapsed_ms = t1.elapsed().as_millis(),
        count = pir_witnesses.len(),
        "PIR witness fetch: queries complete",
    );

    let witnesses = requests
        .iter()
        .zip(pir_witnesses.iter())
        .map(|(req, w)| PirFetchedWitness {
            note_id: req.note_id,
            position: req.position,
            siblings: w.siblings,
            anchor_height: w.anchor_height,
            anchor_root: w.anchor_root,
        })
        .collect();

    tracing::info!(
        total_ms = t0.elapsed().as_millis(),
        num_witnesses = requests.len(),
        "PIR witness fetch: done",
    );
