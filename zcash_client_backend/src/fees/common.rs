...
fn orchard_action_count_from_parts<E, NoteRefT>(
    orchard_pool_restrictions: orchard::bundle::BundleVersion,
    orchard_inputs: usize,
    ironwood_inputs: usize,
    orchard_outputs: usize,
    ironwood_outputs: usize,
) -> Result<(usize, usize), ChangeError<E, NoteRefT>> {
    ...
    E: From<F::Error> + From<BalanceError>,
{
    ...
    let orchard_action_count =
        |change_counts: OutputManifest| -> Result<(usize, usize), ChangeError<E, NoteRefT>> {
    ...
    let (min_orchard_action_count, min_ironwood_action_count) =
    ...
    let min_fee = cfg
        ...
        .fee_required(
            ...
            min_orchard_action_count,
            min_ironwood_action_count,
        )
    ...
    let (mut change, fee) = match total_in.cmp(&total_out_with_min_fee) {
        ...
        _ => {
            let (target_orchard_action_count, target_ironwood_action_count) =
            ...
            let max_fee = max(
                ...
                cfg.fee_rule
                    .fee_required(
                        ...
                        target_orchard_action_count,
                        target_ironwood_action_count,
                    )
                    .map_err(|fee_error| ChangeError::StrategyError(E::from(fee_error)))?,
            );
            ...
            let total_fee = if split_count < target_change_count {
                ...
                let (split_orchard_action_count, split_ironwood_action_count) =
                ...
                cfg.fee_rule
                    .fee_required(
                        ...
                        split_orchard_action_count,
                        split_ironwood_action_count,
                    )
                    .map_err(|fee_error| ChangeError::StrategyError(E::from(fee_error)))?
            } else {
                max_fee
...
pub(crate) fn check_for_uneconomic_inputs<NoteRefT: Clone, E>(
    ...
    sapling: &impl sapling_fees::BundleView<NoteRefT>,
    #[cfg(feature = "orchard")] orchard: &impl orchard_fees::BundleView<NoteRefT>,
    #[cfg(feature = "orchard")] orchard_pool_restrictions: orchard::bundle::BundleVersion,
    #[cfg(zcash_unstable = "nu6.3")] orchard_outputs_are_ironwood: bool,
    marginal_fee: Zatoshis,
    grace_actions: usize,
    ...
    let allowed_dust = |change: &OutputManifest| {
        ...
        let hypothetical_actions = |t_extra, s_extra, _o_extra: Option<bool>| {
            ...
            #[cfg(feature = "orchard")]
            let o_action_count = o_action_count.0 + o_action_count.1;
            #[cfg(not(feature = "orchard"))]
