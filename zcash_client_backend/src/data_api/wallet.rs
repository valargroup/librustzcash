    ...
    ChangeT: ChangeStrategy<MetaSource = DbT>,
{
    ...
    tracing::info!(
        "[PIR-DEBUG] propose_transfer: account={:?}, confirmations_policy={:?}",
        spend_from_account,
    ...
    FeeRuleT: FeeRule,
{
    ...
    tracing::info!(
        "[PIR-DEBUG] create_proposed_transactions: use_pir_witnesses={}, steps={}",
        use_pir_witnesses,
        proposal.steps().len()
    );
        ...
        if proposal_step.involves(PoolType::Shielded(ShieldedProtocol::Orchard)) {
            ...
            if use_pir_witnesses {
                let pir_result = match proposal_step.shielded_inputs() {
                    Some(inputs) => {
                        let result = pir_orchard_witnesses(wallet_db, inputs)?;
                        tracing::info!(
                            "[PIR-DEBUG] Using PIR witnesses for Orchard: {} inputs, anchor present={}",
                            result.1.len(),
                            result.0.is_some()
                        );
                        result
