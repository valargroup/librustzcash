...
/// A process-wide cache of the Orchard proving key, keyed by circuit version.
///
/// `orchard::circuit::ProvingKey::build` takes several seconds and was
/// previously called on every `Builder::build`, even though the key depends
/// only on the circuit version (constant for a chain). There are three circuit
/// versions, so one `OnceLock` each suffices, and the leaked `'static`
/// reference is exactly what `create_proof` wants.
fn cached_orchard_proving_key(
    version: orchard::circuit::OrchardCircuitVersion,
) -> &'static orchard::circuit::ProvingKey {
    use orchard::circuit::OrchardCircuitVersion::*;
    use std::sync::OnceLock;
    static INSECURE_PRE_NU6_2: OnceLock<orchard::circuit::ProvingKey> = OnceLock::new();
    static FIXED_POST_NU6_2: OnceLock<orchard::circuit::ProvingKey> = OnceLock::new();
    static POST_NU6_3: OnceLock<orchard::circuit::ProvingKey> = OnceLock::new();
    let slot = match version {
        InsecurePreNu6_2 => &INSECURE_PRE_NU6_2,
        FixedPostNu6_2 => &FIXED_POST_NU6_2,
        PostNu6_3 => &POST_NU6_3,
    };
    slot.get_or_init(|| orchard::circuit::ProvingKey::build(version))
}
...
impl<P: consensus::Parameters, U: sapling::builder::ProverProgress> Builder<P, U> {
        ...
        OP: OutputProver,
    {
        ...
        // The Orchard proving key depends only on the circuit version, which is
        // constant across every transaction on a given chain, but building it
        // takes several seconds. Building it per `build()` call dominated the
        // cost of small transactions. Cache it per circuit version instead.
        let orchard_proving_key = {
            let need = unauthed_tx.orchard_bundle.is_some()
                || unauthed_tx.ironwood_bundle.is_some();
            need.then(|| {
