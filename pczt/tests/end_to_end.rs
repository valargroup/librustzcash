...
/// Builds and IO-finalizes a real single-spend, two-output Orchard PCZT, returning it
/// together with its spend authorizing key and the spend action index.
///
/// Used by the recompute-and-fill tests, which need a PCZT whose six derived Orchard fields
/// hold genuine cryptographic values so that recompute can be checked to reproduce them
/// byte-for-byte.
fn io_finalized_orchard_pczt() -> (Pczt, orchard::keys::SpendAuthorizingKey, usize) {
    let mut rng = OsRng;
    ...
    // The recompute-and-fill path assumes the Ironwood migration's empty memo, which is the
    // raw `[0u8; 512]` plaintext memo (as the orchard restricted builder uses), NOT the
    // ZIP 302 empty memo `MemoBytes::empty()` (lead byte `0xF6`). Build these outputs with a
    // `[0u8; 512]` memo so the recomputed `enc_ciphertext` matches byte-for-byte.
    let zero_memo = MemoBytes::from_bytes(&[0u8; 512]).unwrap();
    builder
    ...
    (pczt, orchard_ask, spend_index)
}

/// The byte encodings of an action's six derived Orchard-shaped fields, as the wire format
/// carries them.
...
/// Snapshots the six derived Orchard-shaped fields of every action in an Orchard-shaped
/// bundle.
fn bundle_derived_fields(bundle: &pczt::orchard::Bundle) -> Vec<DerivedFieldBytes> {
    bundle
        .actions()
        .iter()
...
/// Snapshots the six derived Orchard-shaped fields of every action in a PCZT's `orchard`
/// bundle.
fn orchard_derived_fields(pczt: &Pczt) -> Vec<DerivedFieldBytes> {
    bundle_derived_fields(pczt.orchard())
}
...
/// (c) recompute round-trip: starting from a full PCZT, clearing ALL SIX derived Orchard
/// fields, serializing as v3 and parsing, then driving the Prover (whose `into_parsed`
/// recompute-and-fill runs before it re-serializes the bundle) reproduces every cleared
/// field byte-for-byte. The resulting PCZT also still proves, signs, and extracts.
#[test]
fn recompute_round_trip_refills_all_six_orchard_fields() {
    let (full, orchard_ask, spend_index) = io_finalized_orchard_pczt();
    let original_fields = orchard_derived_fields(&full);
    // Sanity: every derived field is present in the full PCZT.
    ...
    // Clear all six derived fields on every action.
    let redacted = pczt::roles::redactor::Redactor::new(full.clone())
        .redact_orchard_with(|mut r| {
            r.redact_actions(|mut a| {
                ...
                a.clear_enc_ciphertext();
            });
        })
        .finish();

    // A redacted PCZT must serialize as v3 (v2 cannot represent the omissions), and parse
    // back to a PCZT whose six fields are still absent (parse does NOT recompute).
    let encoded = redacted.serialize();
    assert_eq!(
        u32::from_le_bytes(encoded[4..8].try_into().unwrap()),
        3,
        "a redacted PCZT must use the v3 header",
    );
    // `serialize()` content-gates to v3 here; `serialize_v3()` forces it unconditionally.
    assert_eq!(encoded, redacted.serialize_v3());
    let reparsed = Pczt::parse(&encoded).unwrap();
    for f in orchard_derived_fields(&reparsed) {
    ...
    // Drive the Prover: its `into_parsed` recompute-and-fill runs, then `serialize_from`
    // writes the recomputed values back into the bundle.
    let refilled = Prover::new(reparsed)
        .create_orchard_proof(orchard_proving_key())
        .unwrap()
        .finish();

    // The recomputed fields are byte-identical to the originals.
    assert_eq!(orchard_derived_fields(&refilled), original_fields);

    // And the recomputed PCZT still completes signing and extraction.
    let mut signer = Signer::new(refilled).unwrap();
...
/// `Pczt::fill_derived_fields` repopulates the six derived Orchard fields in place (the
/// inverse of the redactor's `clear_*`), directly on the wire format without parsing or
/// proving. This is the path the Keystone firmware uses to refill an inbound-redacted PCZT
/// before it reads the raw fields for the sighash.
#[test]
fn fill_derived_fields_refills_all_six_orchard_fields_in_place() {
    let (full, _orchard_ask, _spend_index) = io_finalized_orchard_pczt();
    let original_fields = orchard_derived_fields(&full);

    // Filling an already-complete PCZT leaves every field unchanged.
    let mut already_full = full.clone();
    already_full.fill_derived_fields().unwrap();
    assert_eq!(orchard_derived_fields(&already_full), original_fields);

    // Clear all six on every action, then reparse so they are absent on the wire.
    let redacted = pczt::roles::redactor::Redactor::new(full.clone())
        .redact_orchard_with(|mut r| {
            r.redact_actions(|mut a| {
                a.clear_cv_net();
        ...
        .finish();
    let mut reparsed = Pczt::parse(&redacted.serialize()).unwrap();
    for f in orchard_derived_fields(&reparsed) {
        assert_eq!(f.cv_net, None);
        assert_eq!(f.enc_ciphertext, None);
    }

    // Filling reproduces every cleared field byte-for-byte.
    reparsed.fill_derived_fields().unwrap();
    assert_eq!(orchard_derived_fields(&reparsed), original_fields);
}

/// (d) ordering: clearing the spend `nullifier` AND the output `cmx` together still
/// reconstructs both correctly. This exercises the ordering hazard, because the output's
/// `cmx` is derived from a `rho` that comes from the (also-cleared) spend nullifier, so the
/// nullifier must be recomputed first.
#[test]
fn recompute_handles_nullifier_then_cmx_ordering() {
    let (full, _orchard_ask, _spend_index) = io_finalized_orchard_pczt();
    let original_fields = orchard_derived_fields(&full);
    ...
    let reparsed = Pczt::parse(&redacted.serialize()).unwrap();
    for f in orchard_derived_fields(&reparsed) {
        assert_eq!(f.nullifier, None);
        assert_eq!(f.cmx, None);
        // The four untouched fields are still present.
        assert!(f.cv_net.is_some());
        assert!(f.rk.is_some());
        assert!(f.ephemeral_key.is_some());
        assert!(f.enc_ciphertext.is_some());
    ...
    // Both the nullifier and the cmx (which depends on the nullifier-derived rho) are
    // reconstructed to exactly their original values.
    assert_eq!(orchard_derived_fields(&refilled), original_fields);
}
...
/// Recompute round-trip for an Ironwood (V3) bundle. This is the load-bearing check for the
/// `note_version` threading: Ironwood outputs are V3, so the recomputed `enc_ciphertext` must
/// use the V3 (Ironwood) note encryption domain, not the V2 (Orchard) one. Clearing all six
/// derived fields and driving `create_ironwood_proof` (whose `into_parsed` recompute runs
/// first) must reproduce them byte-for-byte.
#[cfg(zcash_unstable = "nu6.3")]
#[test]
fn recompute_round_trip_refills_ironwood_v3_fields() {
    let mut rng = OsRng;
    ...
    // `[0u8; 512]` raw memo (the migration encoding the recompute assumes), not `0xF6`.
    let zero_memo = MemoBytes::from_bytes(&[0u8; 512]).unwrap();
    let mut builder = Builder::new(
        nu6_3_network(),
        10_000_000.into(),
    ...
    let pczt = Creator::build_from_parts(pczt_parts).unwrap();
    let full = IoFinalizer::new(pczt).finalize_io().unwrap();
    let original_fields = bundle_derived_fields(full.ironwood());
    assert!(!original_fields.is_empty());

    // Clear all six derived fields on every Ironwood action.
    let redacted = pczt::roles::redactor::Redactor::new(full.clone())
    ...
    let reparsed = Pczt::parse(&redacted.serialize()).unwrap();
    for f in bundle_derived_fields(reparsed.ironwood()) {
        assert_eq!(f.cv_net, None);
        assert_eq!(f.nullifier, None);
        assert_eq!(f.rk, None);
        assert_eq!(f.cmx, None);
    ...
    // Drive the Ironwood Prover, which recomputes the cleared V3 fields before re-serializing.
    let refilled = Prover::new(reparsed)
        .create_ironwood_proof(ironwood_proving_key())
        .unwrap()
        .finish();
    assert_eq!(bundle_derived_fields(refilled.ironwood()), original_fields);

    // And it still signs and extracts.
    let index = ironwood_meta.spend_action_index(0).unwrap();
    let mut signer = Signer::new(refilled).unwrap();
    signer.sign_ironwood(index, &orchard_ask).unwrap();
    let tx = TransactionExtractor::new(signer.finish()).extract().unwrap();
    assert!(tx.ironwood_bundle().is_some());
}
