...
fn orchard_zip32_derivation(
    seed_fingerprint: [u8; 32],
    account_index: zip32::AccountId,
) -> (orchard::pczt::Zip32Derivation, Vec<u32>) {
...
fn compact_migration_child_pczt_with_zip32() -> (
    Pczt,
...
fn compact_migration_child_pczt_with_zip32_at_scope(
    ...
    assert_eq!(spend_index, 0);
    assert_eq!(pczt.orchard().actions().len(), 1);
    assert_eq!(pczt.ironwood().actions().len(), 1);

    let (zip32_derivation, derivation_path) =
        orchard_zip32_derivation(seed_fingerprint, account_index);
    let pczt = Updater::new(pczt)
...
#[test]
fn compact_migration_child_decodes_to_signable_pczt() {
    let (full, orchard_ask, orchard_fvk, seed_fingerprint, derivation_path, spend_index) =
        compact_migration_child_pczt_with_zip32();
    let full_orchard_fields = derived_fields(full.orchard());
    let full_ironwood_fields = derived_fields(full.ironwood());
    let full_sighash = Signer::new(full.clone()).unwrap().shielded_sighash();

    let payload = pczt::compact_migration::encode_child(&full).unwrap();
    assert!(payload.starts_with(pczt::compact_migration::COMPACT_MIGRATION_CHILD_MAGIC));
    assert!(
        payload.len() < 1_400,
        "compact migration child payload should stay near 1.2 kB while carrying randomized \
         Orchard dummy ciphertexts, got {} bytes",
        payload.len()
    );
    ...
    assert_eq!(
        ...
        1,
        "compact decoded child should refill only the real Orchard spend FVK"
    );
    decoded.fill_derived_fields().unwrap();

    assert_eq!(derived_fields(decoded.orchard()), full_orchard_fields);
    assert_eq!(derived_fields(decoded.ironwood()), full_ironwood_fields);
    assert_eq!(
        Signer::new(decoded.clone()).unwrap().shielded_sighash(),
        full_sighash,
        "compact decoded child must produce the same shielded sighash"
    );

    let sig_seed = [9; 32];
    let expected_sig = expected_spend_auth_sig(
        &full,
        false,
        spend_index,
        &orchard_ask,
    ...
    let signed = low_level_signer::Signer::new(decoded)
        .sign_orchard_with::<low_level_signer::OrchardParseError, _>(|_, bundle, _| {
            assert!(bundle.actions()[spend_index].spend().fvk().is_none());
            bundle.actions_mut()[spend_index]
                .sign(full_sighash, &orchard_ask, ChaCha20Rng::from_seed(sig_seed))
                .expect("signing succeeds");
            Ok(())
        })
        .unwrap()
        .finish();
    let produced_sig = signed.orchard().actions()[spend_index]
        .spend()
        .spend_auth_sig()
        .expect("action was signed");
    assert_eq!(produced_sig, expected_sig);
    assert_valid_spend_auth_sig(
        signed.orchard().actions()[spend_index]
            .spend()
            .rk()
            .as_ref()
            .expect("signing populates `rk`"),
    ...
    let combined = Combiner::new(vec![full.clone(), signed])
        .combine()
        .expect("signed compact child must recombine with the original full PCZT");
    assert_eq!(combined.orchard().anchor(), full.orchard().anchor());
    assert_eq!(combined.ironwood().anchor(), full.ironwood().anchor());
    assert!(
        combined.orchard().actions()[spend_index]
            .spend()
            .spend_auth_sig()
            .is_some(),
...
#[test]
fn compact_migration_batch_decodes_virtual_children() {
    let first = compact_migration_child_pczt_with_zip32();
    let second = compact_migration_child_pczt_with_zip32();
    let child_payloads = [
        pczt::compact_migration::encode_child(&first.0).unwrap(),
        pczt::compact_migration::encode_child(&second.0).unwrap(),
    ];
    let child_refs = child_payloads.iter().map(Vec::as_slice).collect::<Vec<_>>();

    let payload = pczt::compact_migration::encode_batch_from_child_payloads(1, &child_refs)
        .expect("children share one global header and address");
    assert!(payload.starts_with(pczt::compact_migration::COMPACT_MIGRATION_BATCH_MAGIC));

    let decoded = pczt::compact_migration::decode_batch_to_pczt_bytes(&payload).unwrap();
    assert_eq!(decoded.len(), 2);
    assert_eq!(decoded[0].0, b"migration-1");
    assert_eq!(decoded[1].0, b"migration-2");

    for (decoded, original) in decoded.into_iter().zip([first, second]) {
        let mut decoded_pczt = Pczt::parse(&decoded.1).unwrap();
        assert_eq!(
            decoded_pczt.fill_missing_spend_fvks_for_zip32_path(
                &original.3,
                &original.4,
                original.2,
            ),
        ...
        decoded_pczt.fill_derived_fields().unwrap();
        assert_eq!(
            derived_fields(decoded_pczt.orchard()),
            derived_fields(original.0.orchard())
        );
        assert_eq!(
            derived_fields(decoded_pczt.ironwood()),
            derived_fields(original.0.ironwood())
        );
        assert_eq!(
            Signer::new(decoded_pczt).unwrap().shielded_sighash(),
            Signer::new(original.0).unwrap().shielded_sighash()
        );
    }
}
